...
自定义提供程序最简单的形式就是一个标准 jar 文件，其中包含一个或多个服务实现。启动时，Keycloak 会扫描其 classpath，并使用标准的 java.util.ServiceLoader 机制选择所有可用的提供程序。这意味着我们要做的就是在 jar 的 META-INF/services 文件夹中创建一个以我们要提供的特定服务接口命名的文件，并在其中写入我们的实现的完全合格名称。

但是，我们能为 Keycloak 添加什么样的服务呢？如果我们进入 Keycloak 管理控制台的服务器信息页面，就会看到很多服务：

Keycloak 服务器信息提供商 1
在这张图中，左边一栏对应的是某个服务提供商接口（Service Provider Interface，简称 SPI），右边一栏显示的是该 SPI 的可用提供商。
...
org.keycloak.events.EventListenerProviderFactory： 创建监听 Keycloak 事件的提供程序。EventType Javadoc 页面包含提供程序可处理的可用事件自定义列表。使用此 SPI 的典型用途是创建审计数据库
org.keycloak.adapters.saml.RoleMappingsProvider： 将从外部身份提供程序接收的 SAML 角色映射到 Keycloak 的角色。这种映射非常灵活，允许我们在给定 "境界 "的上下文中重命名、删除和/或添加角色。
...
创建表 if not exists users(
    username varchar(64) not null 主键、
...
我们的自定义提供程序项目只是一个创建 jar 文件的普通 Maven 项目。为了避免将我们的提供程序编译-部署-重启循环到普通 Keycloak 实例的耗时过程，我们将使用一个不错的技巧：将 Keycloak 作为测试时依赖项嵌入我们的项目。
...
<依赖关系
    <群组Id>com.baeldung</群组Id
    <artifactId>oauth-authorization-server</artifactId> <版本>0.1.0</版本
    <版本>0.1.0-SNAPSHOT</版本
    <范围>测试</范围
</ 依赖关系
...
create()： 返回实际的提供程序实现。
Keycloak 会为每个事务调用 create() 方法，并将 KeycloakSession 和 ComponentModel 作为参数传递。在这里，事务指的是任何需要访问用户存储空间的操作。最典型的例子就是登录流程：在某个时刻，Keycloak 会调用给定 "境界 "的每个已配置用户存储来验证凭证。因此，我们应该避免在此时执行任何代价高昂的初始化操作，因为 create() 方法会一直被调用。

尽管如此，实现起来还是很简单的：

公共类 CustomUserStorageProviderFactory
  实现 UserStorageProviderFactory<CustomUserStorageProvider> {
    @Override
    public String getId() {
...
keycloak 服务器信息提供者 pic2
该文件的内容只是 SPI 实现的全称：

# SPI 类实现
com.baeldung.auth.provider.user.CustomUserStorageProviderFactory
3.3. UserStorageProvider 实现

乍一看，UserStorageProvider 的实现并不像我们想象的那样。它只包含几个回调方法，其中没有一个与实际用户有关。这是因为 Keycloak 希望我们的提供程序也能实现其他支持特定用户管理方面的混合接口。

Keycloak 文档中提供了可用接口的完整列表，这些接口被称为 "提供程序能力"（Provider Capabilities）。对于一个简单的只读提供程序，我们需要实现的唯一接口就是 UserLookupProvider。它只提供查找功能，这意味着 Keycloak 会在需要时自动将用户导入其内部数据库。不过，原始用户的密码不会用于身份验证。为此，我们还需要实现 CredentialInputValidator。
...
此外，虽然这里没有显示，但我们还可以使用 onCreated() 方法附加逻辑，每次管理员将我们的提供程序添加到 "境界 "时都会执行该逻辑。这样，我们就可以执行一次性初始化逻辑，为使用存储做好准备，这在某些情况下可能是必要的。例如，我们可以使用此方法修改数据库，添加一列来记录特定用户是否已经使用过 Keycloak。
...
正如我们所见，这里没有什么特别之处：只是普通的 JDBC 代码。值得一提的实现注意事项： UserQueryProvider 方法通常有分页和非分页两种版本。由于用户存储可能会有大量记录，因此非分页版本应使用合理的默认值，简单地委托给分页版本。更妙的是，我们可以添加一个配置参数来定义什么是 "合理的默认值"。
...
keycloak 服务器信息提供程序 pic3
要访问管理控制台，我们需要使用管理员凭证，可以通过查看 application-test.yml 文件获得。登录后，让我们进入 "服务器信息 "页面：

keycloak 服务器信息提供商 pic4
在 "提供商 "选项卡上，我们可以看到自定义提供商与其他内置存储提供商一起显示：

keycloak 服务器信息提供商照片5
我们还可以检查 Baeldung realm 是否已在使用此提供程序。为此，我们可以在左上角的下拉菜单中选择它，然后导航到用户联盟页面：

keycloak 服务器信息提供程序 pic6-1
